...
def scrape(request, title):
    ...
    def run():
        ...
        if Movie.objects.filter(title=title).count() == 0:
            ...
            # handle possible title miss
            # create template / call models / render to template

            # for now handle movies with one instance
            m = Movie(title=details.get('title'))
            m.save()
        else:
            m = Movie.objects.get(title)
